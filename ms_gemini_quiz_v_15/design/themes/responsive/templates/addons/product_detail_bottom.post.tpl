...
(function(_, $) {
    ...
    let loadingTimer; // Переменная для таймера долгого ожидания
    ...
    function showTypingIndicator() {
        let indicator = `<div class="msg-row ai-row" id="typing_indicator">
            <div class="msg-bubble ai-bubble typing" id="typing_content"><span class="dot"></span><span class="dot"></span><span class="dot"></span></div>
        </div>`;
        $('#quiz_chat_history').append(indicator);
        scrollToBottom();

        // УМНЫЙ UX: Если ответ не приходит дольше 4 секунд, значит нейросеть начала клонировать товар
        loadingTimer = setTimeout(function() {
            $('#typing_content').removeClass('typing');
            $('#typing_content').html('<span class="loading-pulse">⏳ Проводим финальный анализ и генерируем персональную спецификацию. Это займет несколько секунд...</span>');
            scrollToBottom();
        }, 4000);
    }
    ...
    function sendChatRequest(text, isInit = false) {
        ...
        $.ceAjax('request', fn_url('gemini_quiz.process_chat'), {
            ...
            callback: function(response) {
                // Очищаем таймер и удаляем индикатор, как только пришел ответ
                ...
                if (response.status === 'success') {
                    ...
                    if (response.redirect_product_id && !isFeatureSaved) {
                        isFeatureSaved = true;
                        $('#quiz_input_zone').hide();
                        $('#quiz_chat_history').append(`<div class="msg-row ai-row animation-fade-up"><div class="msg-bubble ai-bubble final-gold-bubble">✓ Готово! Перенаправляем на вашу персональную страницу...</div></div>`);
...
@keyframes textPulse {
    0% { opacity: 0.5; }
    50% { opacity: 1; text-shadow: 0 0 8px rgba(223,186,115,0.4); }
    100% { opacity: 0.5; }
}
</style>
{/if}